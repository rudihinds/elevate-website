...
export const MacbookScroll = ({
  ...
  const translate = useTransform(scrollYProgress, [0, 0.9], [0, 1500]);
  const rotate = useTransform(scrollYProgress, [0, 0.12, 0.2], [-28, -28, 0]);
  const textTransform = useTransform(scrollYProgress, [0, 0.3], [0, 100]);
  const textOpacity = useTransform(scrollYProgress, [0, 0.2], [1, 0]);

  //min-h-[200vh] missing from top div to give animation space to rotate. fix later by giving shorter min-h

  return (
    <div
      ref={ref}
      className=" min-h-[100rem] flex flex-col items-center py-0 justify-start flex-shrink-0 [perspective:800px] transform md:scale-100  scale-[0.35] sm:scale-50"
    >
      <motion.h2
        style={{
